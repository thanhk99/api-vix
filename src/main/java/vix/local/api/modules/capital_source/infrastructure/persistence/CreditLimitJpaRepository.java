...
public interface CreditLimitJpaRepository extends JpaRepository<CreditLimitEntity, UUID> {
       ...
       Page<CreditLimitEntity> findByContractId(UUID contractId, Pageable pageable);

       @Query("SELECT c FROM CreditLimitEntity c WHERE c.status != 'DELETED' " +
       ...
       java.util.List<CreditLimitEntity> findByContractIdInAndStatusNot(List<UUID> contractIds, String status);

       java.util.List<CreditLimitEntity> findByPartnerIdAndStatus(UUID partnerId, String status);

       java.util.Optional<CreditLimitEntity> findFirstByLimitId(String limitId);
}